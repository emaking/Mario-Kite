...
Function SetGfx()
	...
	image=LoadImage( "_Medias/sprites/groupy2.bmp" )
	If Not image image=LoadImage( "_Medias/sprites/groupy2.bmp" )
	MidHandle image
	
	;font=LoadFont( "ArialBlack",16 )
	;SetFont font
	...
	url$="dav.emaking@gmail.com"
	url_x=600-StringWidth( url$ )
	url_y=470-FontHeight()
	mode=11
	Repeat
		Cls
		...
		Else
			Text tx,ty+FontHeight()*5,GfxModeWidth( mode )+","+GfxModeHeight( mode )+",  : Mode "+GfxModeDepth( mode )+ mode,True
		EndIf
		
		Color 255,0,0
		Text nx,ty+FontHeight()*7,"[Return] to begin",True
		Text tx,ty+FontHeight()*8,"[Arrows] to change mode",True
